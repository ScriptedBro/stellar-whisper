...
pub fn generate_transcript(
    ...
    crate::trace!("===== TRANSCRIPT PARAMETERS =====");
    crate::trace!("eta = 0x{}", crate::ultrahonk::debug::Hex(&rp.eta.to_bytes()));
    crate::trace!("eta_two = 0x{}", crate::ultrahonk::debug::Hex(&rp.eta_two.to_bytes()));
    crate::trace!(
        "eta_three = 0x{}",
        crate::ultrahonk::debug::Hex(&rp.eta_three.to_bytes())
    );
    crate::trace!("beta = 0x{}", crate::ultrahonk::debug::Hex(&rp.beta.to_bytes()));
    crate::trace!("gamma = 0x{}", crate::ultrahonk::debug::Hex(&rp.gamma.to_bytes()));
    crate::trace!("rho = 0x{}", crate::ultrahonk::debug::Hex(&rho.to_bytes()));
    crate::trace!("gemini_r = 0x{}", crate::ultrahonk::debug::Hex(&gemini_r.to_bytes()));
    crate::trace!(
        "shplonk_nu = 0x{}",
        crate::ultrahonk::debug::Hex(&shplonk_nu.to_bytes())
    );
    crate::trace!("shplonk_z = 0x{}", crate::ultrahonk::debug::Hex(&shplonk_z.to_bytes()));
    crate::trace!("circuit_size = {}", circuit_size);
    crate::trace!("public_inputs_total = {}", public_inputs_size);
    crate::trace!("public_inputs_offset = {}", pub_inputs_offset);
    crate::trace!("=================================");
