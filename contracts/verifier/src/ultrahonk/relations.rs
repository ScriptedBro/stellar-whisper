...
use crate::ultrahonk::field::Fr;
use crate::ultrahonk::types::{RelationParameters, Wire, NUMBER_OF_SUBRELATIONS};
use core::ops::Index;
...
fn accumulate_auxiliary_relation(
    ...
    let one = Fr::one(env);
    let limb_size = Fr(Bn254Fr::from_bytes(soroban_sdk::bytesn!(
        &env,
...
mod tests {
    ...
    fn test_relations_determinism() {
        ...
        let mut purported_evaluations =
            Fr::zero_array::<{ crate::ultrahonk::types::NUMBER_OF_ENTITIES }>(&env);
        ...
        let mut alphas = Fr::zero_array::<{ crate::ultrahonk::types::NUMBER_OF_ALPHAS }>(&env);
        ...
        assert_eq!(
            result.to_bytes(),
            crate::ultrahonk::debug::hex_to_bytes(
