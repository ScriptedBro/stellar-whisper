...
pub fn verify_sumcheck(
    env: &Env,
    proof: &crate::ultrahonk::types::Proof,
    tp: &Transcript,
    vk: &VerificationKey,
) -> Result<(), &'static str> {
    ...
    for round in 0..log_n {
        ...
        if !check_sum(round_univariate, round_target.clone()) {
            crate::trace!("Round {} check failed!", round);
            crate::trace!("  round_univariate[0] = 0x{}", crate::ultrahonk::debug::Hex(&round_univariate[0].to_bytes()));
            crate::trace!("  round_univariate[1] = 0x{}", crate::ultrahonk::debug::Hex(&round_univariate[1].to_bytes()));
            let total_sum = &round_univariate[0] + &round_univariate[1];
            crate::trace!("  total sum           = 0x{}", crate::ultrahonk::debug::Hex(&total_sum.to_bytes()));
            crate::trace!("  round_target        = 0x{}", crate::ultrahonk::debug::Hex(&round_target.to_bytes()));
    ...
    if grand_honk_relation_sum == round_target {
        ...
        crate::trace!(
            "grand_relation = 0x{}",
            crate::ultrahonk::debug::Hex(&grand_honk_relation_sum.to_bytes())
        );
        crate::trace!("target = 0x{}", crate::ultrahonk::debug::Hex(&round_target.to_bytes()));
        crate::trace!(
            "difference = 0x{}",
            crate::ultrahonk::debug::Hex(&(grand_honk_relation_sum - round_target).to_bytes())
        );
        crate::trace!("======================================");
